...
export default {
  ...
  computed: {
    ...
    clickStopSwitch() {
      return this.clickStopflag && this.autoPlay && this.scrollSwitch;
    },
  ...
  methods: {
    ...
    clickFn() {
      if (this.clickStopSwitch) this._stopMove();
      else this._startMove();

      this.clickStopflag = !this.clickStopflag;
    },
    touchStart(e) {
    ...
    leave() {
      if (this.hoverStopSwitch && this.clickStopflag) this._startMove();
    },
  ...
  mounted() {
    this._initMove();
    this.$nextTick(() => {
      this.options.clickStop &&
        this.$refs.realBox.addEventListener("click", this.clickFn);
    });
  },
  watch: {
